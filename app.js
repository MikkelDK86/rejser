...
<div class="tabBody" data-body="story"><div class="mrow"><div>${NUM(pad2(days(t)==='—'?0:days(t)),30)}<div class="cap">${tr('m.days')}</div></div><div>${NUM(pad2(cityList(t).length),30)}<div class="cap">${tr('m.places')}</div></div><div>${NUM(pad2(t.photos.length),30)}<div class="cap">${tr('m.photos')}</div></div>${no?`<div>${NUM('№'+pad2(no),30)}<div class="cap">${tr('m.entry')}</div></div>`:`<div>${NUM(daysUntil(t),30)}<div class="cap">${tr('m.togo')}</div></div>`}</div><p class="storyTxt">${esc(t.story||tr('trip.nostory'))}</p><div class="chips">${(t.locations||[]).filter(l=>l.city).map(l=>`<button class="chip link" onclick="openCity('${esc(enc(countryKey(l.country)))}','${esc(enc(cityKey(l.city)))}')">${esc(l.city)}</button>`).join('')}</div>${memoryCard(t)}${t.photos.slice(0,2).map((p,i)=>`<button class="pcard" onclick="openLightbox('${esc(t.id)}',${i})" aria-label="${esc(tr('photo.open',{n:i+1}))}"><img src="${esc(p.thumb)}" alt=""><span class="scrimD"></span><span class="pc"><b>${esc(tr('photo.n',{n:pad2(i+1)}))}</b><span>${esc(t.title)}</span></span></button>`).join('')}</div>
...
function route(){
  ...
  if(name==='country'&&arg){if(renderCountry(dec(arg)))return activate('country')}
  if(name==='city'&&arg&&arg2){if(renderCity(dec(arg),dec(arg2)))return activate('city')}
...
function stampsPanel(pe,n,next){
  ...
  const stamp=(e,dashed)=>`<div class="stampWrap">${!dashed&&latest&&e.key===latest.key?'<span class="blotch"></span>':''}<button class="stampBtn" onclick="openCountry('${esc(enc(e.key))}')">${stampSvg(e,{dashed})}</button>${!dashed&&e.visits>1?`<span class="visitBadge" aria-hidden="true">×${e.visits}</span>`:''}</div>`;
...
function renderCity(ckey,cityK){
  ...
  $('city').innerHTML=`<div class="top"><button class="iconBtn" onclick="goBack()" aria-label="${esc(tr('back'))}">${ico('back')}</button><button class="capBtn" onclick="openCountry('${esc(enc(ckey))}')">${esc(cname)}</button></div><div class="head"><h1 class="h1">${esc(name)}</h1><div class="cap">${esc(cname)}</div></div><div class="mrow padrow"><div>${NUM(past.length?'×'+past.length:'—',44)}<div class="cap">${esc(tr('city.visits'))}</div></div></div><div class="secTitle cap">${esc(tr('city.timeline'))}</div><div class="tl">${items}</div>`;
  return true;
}
...
function openMapSheet(f,view){
  ...
  let acts='';
  if(f.key&&S&&(S.s==='v'||S.s==='u'))acts+=`<button class="primary" onclick="closeSheet();openCountry('${esc(enc(f.key))}')">${esc(tr('map.open'))}</button>`;
  if(f.info&&!S)acts+=`<button class="primary" onclick="mapAddWish('${esc(f.info[0])}')">${esc(tr('map.addwish'))}</button>`;
  if(f.info&&S&&S.s==='w')acts+=`<button class="primary" onclick="closeSheet();openEditor(null,{title:'${esc(f.info[0]).replace(/'/g,'')}',country:'${esc(f.info[0])}'})">${esc(tr('map.plan'))}</button>`;
