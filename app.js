...
const I18N={
 ...
 'stamp.latestTrip':['Yellow highlight · latest journey: {title}','Gul markering · seneste rejse: {title}'],'stamp.reserved':['RESERVED','RESERVERET'],'stamp.aria':['Stamp: {name}{date}{res}','Stempel: {name}{date}{res}'],'stamp.res':[' (reserved)',' (reserveret)'],
 'back':['Back','Tilbage'],'edit':['Edit','Redigér'],'tab.story':['Story','Historie'],'tab.photos':['Photos','Billeder'],'tab.map':['Map','Kort'],'tab.stamp':['Stamp','Stempel'],'tab.plans':['Plans','Planer'],'tab.practical':['Practical','Praktisk'],'sections':['Sections','Sektioner'],
 'plans.title':['Plans for this journey','Planer for rejsen'],'plans.name':['Place or idea','Sted eller idé'],'plans.note':['Note (optional)','Note (valgfri)'],'plans.address':['Address (optional)','Adresse (valgfri)'],'plans.openMap':['Open address in Google Maps','Åbn adressen i Google Maps'],'plans.add':['Add to plans','Tilføj til planer'],'plans.empty':['Save restaurants, areas and other places you want to remember.','Skriv restauranter, områder og andre steder, du gerne vil huske.'],'plans.restaurant':['Restaurant','Restaurant'],'plans.area':['Area','Område'],'plans.other':['Other','Andet'],'plans.remove':['Remove plan item','Slet punkt'],'plans.visited':['Mark as visited','Markér som besøgt'],'plans.unvisit':['Mark as not visited','Markér som ikke besøgt'],'plans.error':['Could not save this plan. Please try again.','Planen kunne ikke gemmes. Prøv igen.'],'plans.edit':['Edit','Redigér'],'plans.save':['Save','Gem'],'plans.cancel':['Cancel','Annullér'],'plans.up':['Move up','Flyt op'],'plans.down':['Move down','Flyt ned'],
 'practical.title':['Practical information','Praktiske oplysninger'],'practical.intro':['Keep hotel details, flight times, contacts and confirmations together with this journey.','Saml hoteloplysninger, flytider, kontakter og bekræftelser med denne rejse.'],'practical.name':['Title, e.g. hotel or flight','Titel, fx hotel eller fly'],'practical.details':['Details, times or confirmation number','Detaljer, tider eller bookingnummer'],'practical.address':['Address (optional)','Adresse (valgfri)'],'practical.add':['Add information','Tilføj oplysning'],'practical.empty':['Add useful details for this journey.','Tilføj praktiske oplysninger til rejsen.'],'practical.stay':['Accommodation','Overnatning'],'practical.flight':['Flight','Fly'],'practical.transport':['Transport','Transport'],'practical.contact':['Contact','Kontakt'],'practical.other':['Other','Andet'],'practical.openMap':['Open address in Google Maps','Åbn adressen i Google Maps'],'practical.edit':['Edit','Redigér'],'practical.remove':['Delete information','Slet oplysning'],'practical.save':['Save','Gem'],'practical.cancel':['Cancel','Annullér'],'practical.error':['Could not save this information. Please try again.','Oplysningen kunne ikke gemmes. Prøv igen.'],
...
function ico(kind,size=22){
  const p={
    ...
    close:'<path d="M6 6l12 12M18 6L6 18"/>',edit:'<path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L8 18l-4 1 1-4z"/>',reset:'<path d="M4 12a8 8 0 1 0 3-6.2M4 4v4.5h4.5"/>'
  }[kind];
...
function stampShape(e,opts={}){
  ...
  const h=hashStr(e.key),shape=h%6,rot=opts.rot!==undefined?opts.rot:((h>>3)%15)-7;
  ...
  if(shape===3){
    const fs=Math.max(12,Math.min(22,Math.round(32-name.length*1.35)));
    return `<svg width="170" height="112" viewBox="0 0 170 112" role="img" aria-label="${esc(aria)}" style="${sty}"><path d="M34 5H136L165 56 136 107H34L5 56Z" fill="none" stroke="${color}" stroke-width="3"${d}/><path d="M40 13H130L155 56 130 99H40L15 56Z" fill="none" stroke="${color}" stroke-width="1.2"${d}/><text x="85" y="35" text-anchor="middle" ${SF} font-size="8" font-weight="700" letter-spacing="2" fill="${color}">${esc(no)}</text><text x="85" y="62" text-anchor="middle" ${SF} font-size="${fs}" font-weight="700" letter-spacing="1.2" fill="${color}">${esc(countryCode(e.name))}</text><text x="85" y="81" text-anchor="middle" ${SF} font-size="9" font-weight="600" letter-spacing="1.4" fill="${color}">${esc((city||name).slice(0,19))}</text><text x="85" y="94" text-anchor="middle" ${SF} font-size="7.5" font-weight="700" letter-spacing="1.2" fill="${color}">${esc(date||'')}</text></svg>`;
  }
  if(shape===4){
    const fs=Math.max(10,Math.min(15,Math.round(24-name.length*.65)));
    return `<svg width="170" height="112" viewBox="0 0 170 112" role="img" aria-label="${esc(aria)}" style="${sty}"><path d="M12 6H158V35C143 38 143 54 158 57V106H12V57C27 54 27 38 12 35Z" fill="none" stroke="${color}" stroke-width="3"${d}/><path d="M20 14H150V32M150 61V98H20V61M20 32V14" fill="none" stroke="${color}" stroke-width="1.2" stroke-dasharray="3 3"/><text x="85" y="36" text-anchor="middle" ${SF} font-size="${fs}" font-weight="700" letter-spacing="1" fill="${color}">${esc(name)}</text><text x="85" y="66" text-anchor="middle" ${SF} font-size="24" font-weight="700" letter-spacing="1.4" fill="${color}">${esc(countryCode(e.name))}</text><text x="85" y="84" text-anchor="middle" ${SF} font-size="8.5" font-weight="600" letter-spacing="1.3" fill="${color}">${esc((city||no).slice(0,20))}</text><text x="85" y="96" text-anchor="middle" ${SF} font-size="8" font-weight="700" letter-spacing="1.1" fill="${color}">${esc(date||'')}</text></svg>`;
  }
  if(shape===5){
    const fs=Math.max(11,Math.min(19,Math.round(29-name.length*1.2))),dots=[0,1,2,3,4,5,6,7].map(i=>{const a=i*Math.PI/4-Math.PI/2,x=85+43*Math.cos(a),y=56+43*Math.sin(a);return `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="1.6" fill="${color}"/>`}).join('');
    return `<svg width="170" height="112" viewBox="0 0 170 112" role="img" aria-label="${esc(aria)}" style="${sty}"><circle cx="85" cy="56" r="51" fill="none" stroke="${color}" stroke-width="3"${d}/><circle cx="85" cy="56" r="43" fill="none" stroke="${color}" stroke-width="1.2" stroke-dasharray="2 3"/>${dots}<path d="M56 73H114" stroke="${color}" stroke-width="1.2"/><text x="85" y="35" text-anchor="middle" ${SF} font-size="8" font-weight="700" letter-spacing="1.8" fill="${color}">${esc(no)}</text><text x="85" y="60" text-anchor="middle" ${SF} font-size="${fs}" font-weight="700" letter-spacing="1.1" fill="${color}">${esc(name.length>13?countryCode(e.name):name)}</text><text x="85" y="87" text-anchor="middle" ${SF} font-size="8.5" font-weight="600" letter-spacing="1.3" fill="${color}">${esc((city||date||'').slice(0,18))}</text></svg>`;
  }
...
/* Trip detail */
let tripTabName='story',editingPlanId=null,editingPracticalId=null;
function plansHtml(t){
  const items=Array.isArray(t.plans)?t.plans:[];
  const ordered=[...items];
  const rows=ordered.map((p,i)=>`<article class="planItem${p.done?' done':''}"><button class="planCheck" data-trip="${esc(t.id)}" data-item="${esc(p.id)}" onclick="togglePlanItem(this.dataset.trip,this.dataset.item)" aria-label="${esc(p.done?tr('plans.unvisit'):tr('plans.visited'))}" aria-pressed="${p.done?'true':'false'}">${p.done?'✓':''}</button>${editingPlanId===p.id?`<form class="planEditForm" data-trip="${esc(t.id)}" data-item="${esc(p.id)}" onsubmit="updatePlanItem(event)"><input name="name" value="${esc(p.name)}" maxlength="100" required aria-label="${esc(tr('plans.name'))}"><select name="type" aria-label="${esc(tr('plans.title'))}"><option value="restaurant"${p.type==='restaurant'?' selected':''}>${esc(tr('plans.restaurant'))}</option><option value="area"${p.type==='area'?' selected':''}>${esc(tr('plans.area'))}</option><option value="other"${p.type==='other'?' selected':''}>${esc(tr('plans.other'))}</option></select><input name="address" value="${esc(p.address||'')}" maxlength="240" placeholder="${esc(tr('plans.address'))}" aria-label="${esc(tr('plans.address'))}"><input name="note" value="${esc(p.note||'')}" maxlength="240" placeholder="${esc(tr('plans.note'))}" aria-label="${esc(tr('plans.note'))}"><div class="actions"><button type="button" class="secondary" onclick="cancelPlanEdit()">${esc(tr('plans.cancel'))}</button><button type="submit" class="primary">${esc(tr('plans.save'))}</button></div></form>`:`<div class="planCopy"><div class="planTitle">${esc(p.name)}</div><div class="planType">${esc(tr('plans.'+(['restaurant','area','other'].includes(p.type)?p.type:'other')))}</div>${p.address?`<a class="planAddress" href="${esc(googleMapsUrl(p.address))}" target="_blank" rel="noopener noreferrer" aria-label="${esc(tr('plans.openMap'))}: ${esc(p.address)}">${esc(p.address)} ↗</a>`:''}${p.note?`<div class="planNote">${esc(p.note)}</div>`:''}</div><div class="planActions"><button class="planAction" data-item="${esc(p.id)}" onclick="editPlanItem(this.dataset.item)" aria-label="${esc(tr('plans.edit'))}">${ico('edit',17)}</button><button class="planAction" data-trip="${esc(t.id)}" data-item="${esc(p.id)}" onclick="movePlanItem(this.dataset.trip,this.dataset.item,-1)" aria-label="${esc(tr('plans.up'))}"${i===0?' disabled':''}>↑</button><button class="planAction" data-trip="${esc(t.id)}" data-item="${esc(p.id)}" onclick="movePlanItem(this.dataset.trip,this.dataset.item,1)" aria-label="${esc(tr('plans.down'))}"${i===ordered.length-1?' disabled':''}>↓</button><button class="planDelete" data-trip="${esc(t.id)}" data-item="${esc(p.id)}" onclick="deletePlanItem(this.dataset.trip,this.dataset.item)" aria-label="${esc(tr('plans.remove'))}">${ico('close',18)}</button></div>`}</article>`).join('');
  return `<div class="head" style="padding-top:0;padding-bottom:16px"><h2 class="h1" style="font-size:24px">${esc(tr('plans.title'))}</h2></div><form class="planForm" onsubmit="addPlanItem(event)"><input name="name" maxlength="100" required placeholder="${esc(tr('plans.name'))}" aria-label="${esc(tr('plans.name'))}"><select name="type" aria-label="${esc(tr('plans.title'))}"><option value="restaurant">${esc(tr('plans.restaurant'))}</option><option value="area">${esc(tr('plans.area'))}</option><option value="other">${esc(tr('plans.other'))}</option></select><input name="address" maxlength="240" placeholder="${esc(tr('plans.address'))}" aria-label="${esc(tr('plans.address'))}"><input name="note" maxlength="240" placeholder="${esc(tr('plans.note'))}" aria-label="${esc(tr('plans.note'))}"><button class="primary" type="submit">${esc(tr('plans.add'))}</button></form><div class="planList" style="padding:0 20px 22px">${rows||`<div class="empty small">${esc(tr('plans.empty'))}</div>`}</div>`;
}
function googleMapsUrl(address){return 'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(String(address||''))}
async function savePlanChange(id,change){
  const old=trips.find(x=>x.id===id);if(!old)return;
  const trip={...old,plans:change(Array.isArray(old.plans)?old.plans:[])};
  try{await commitTrip(trip,[],[])}catch(e){toast(tr('plans.error'));return}
  trips=trips.map(x=>x.id===id?trip:x);renderTrip(id);
}
function currentDetailTripId(){return dec(location.hash.split('/')[2]||'')}
async function addPlanItem(event){
  event.preventDefault();const form=event.currentTarget,name=form.elements.namedItem('name').value.trim();if(!name)return;
  const item={id:uid('plan_'),name,type:form.elements.namedItem('type').value,address:form.elements.namedItem('address').value.trim(),note:form.elements.namedItem('note').value.trim(),done:false};
  await savePlanChange(currentDetailTripId(),items=>[...items,item]);
}
async function togglePlanItem(id,itemId){await savePlanChange(id,items=>items.map(p=>p.id===itemId?{...p,done:!p.done}:p))}
async function deletePlanItem(id,itemId){await savePlanChange(id,items=>items.filter(p=>p.id!==itemId))}
function editPlanItem(id){editingPlanId=id;renderTrip(currentDetailTripId());tripTabSet('plans',true)}
function cancelPlanEdit(){editingPlanId=null;renderTrip(currentDetailTripId());tripTabSet('plans',true)}
async function updatePlanItem(event){event.preventDefault();const f=event.currentTarget,id=f.dataset.trip,itemId=f.dataset.item,name=f.elements.namedItem('name').value.trim();if(!name)return;const type=f.elements.namedItem('type').value,address=f.elements.namedItem('address').value.trim(),note=f.elements.namedItem('note').value.trim();editingPlanId=null;await savePlanChange(id,items=>items.map(p=>p.id===itemId?{...p,name,type,address,note}:p))}
async function movePlanItem(id,itemId,delta){await savePlanChange(id,items=>{const i=items.findIndex(p=>p.id===itemId),j=i+delta;if(i<0||j<0||j>=items.length)return items;const next=[...items];[next[i],next[j]]=[next[j],next[i]];return next})}
function practicalHtml(t){
  const items=Array.isArray(t.practical)?t.practical:[];
  const options=(selected='other')=>[['stay','practical.stay'],['flight','practical.flight'],['transport','practical.transport'],['contact','practical.contact'],['other','practical.other']].map(([v,k])=>`<option value="${v}"${selected===v?' selected':''}>${esc(tr(k))}</option>`).join('');
  const rows=items.map(p=>`<article class="planItem"><div class="planCopy">${editingPracticalId===p.id?`<form class="planEditForm" data-trip="${esc(t.id)}" data-item="${esc(p.id)}" onsubmit="updatePracticalItem(event)"><input name="name" value="${esc(p.name)}" maxlength="100" required aria-label="${esc(tr('practical.name'))}"><select name="type" aria-label="${esc(tr('practical.title'))}">${options(p.type)}</select><textarea name="details" maxlength="2000" placeholder="${esc(tr('practical.details'))}" aria-label="${esc(tr('practical.details'))}">${esc(p.details||'')}</textarea><input name="address" value="${esc(p.address||'')}" maxlength="240" placeholder="${esc(tr('practical.address'))}" aria-label="${esc(tr('practical.address'))}"><div class="actions"><button type="button" class="secondary" onclick="cancelPracticalEdit()">${esc(tr('practical.cancel'))}</button><button type="submit" class="primary">${esc(tr('practical.save'))}</button></div></form>`:`<div class="planTitle">${esc(p.name)}</div><div class="planType">${esc(tr('practical.'+(['stay','flight','transport','contact','other'].includes(p.type)?p.type:'other')))}</div>${p.details?`<div class="planNote">${esc(p.details)}</div>`:''}${p.address?`<a class="planAddress" href="${esc(googleMapsUrl(p.address))}" target="_blank" rel="noopener noreferrer" aria-label="${esc(tr('practical.openMap'))}: ${esc(p.address)}">${esc(p.address)} ↗</a>`:''}`}</div>${editingPracticalId===p.id?'':`<div class="planActions"><button class="planAction" data-item="${esc(p.id)}" onclick="editPracticalItem(this.dataset.item)" aria-label="${esc(tr('practical.edit'))}">${ico('edit',17)}</button><button class="planDelete" data-trip="${esc(t.id)}" data-item="${esc(p.id)}" onclick="deletePracticalItem(this.dataset.trip,this.dataset.item)" aria-label="${esc(tr('practical.remove'))}">${ico('close',18)}</button></div>`}</article>`).join('');
  return `<div class="head" style="padding-top:0;padding-bottom:8px"><h2 class="h1" style="font-size:24px">${esc(tr('practical.title'))}</h2><div class="small">${esc(tr('practical.intro'))}</div></div><form class="planForm" onsubmit="addPracticalItem(event)"><input name="name" maxlength="100" required placeholder="${esc(tr('practical.name'))}" aria-label="${esc(tr('practical.name'))}"><select name="type" aria-label="${esc(tr('practical.title'))}">${options()}</select><textarea name="details" maxlength="2000" placeholder="${esc(tr('practical.details'))}" aria-label="${esc(tr('practical.details'))}"></textarea><input name="address" maxlength="240" placeholder="${esc(tr('practical.address'))}" aria-label="${esc(tr('practical.address'))}"><button class="primary" type="submit">${esc(tr('practical.add'))}</button></form><div class="planList" style="padding:0 20px 22px">${rows||`<div class="empty small">${esc(tr('practical.empty'))}</div>`}</div>`;
}
async function savePracticalChange(id,change){
  const old=trips.find(x=>x.id===id);if(!old)return;
  const trip={...old,practical:change(Array.isArray(old.practical)?old.practical:[])};
  try{await commitTrip(trip,[],[])}catch(e){toast(tr('practical.error'));return}
  trips=trips.map(x=>x.id===id?trip:x);renderTrip(id);
}
async function addPracticalItem(event){event.preventDefault();const f=event.currentTarget,name=f.elements.namedItem('name').value.trim();if(!name)return;const item={id:uid('info_'),name,type:f.elements.namedItem('type').value,details:f.elements.namedItem('details').value.trim(),address:f.elements.namedItem('address').value.trim()};await savePracticalChange(currentDetailTripId(),items=>[...items,item])}
function editPracticalItem(id){editingPracticalId=id;renderTrip(currentDetailTripId());tripTabSet('practical',true)}
function cancelPracticalEdit(){editingPracticalId=null;renderTrip(currentDetailTripId());tripTabSet('practical',true)}
async function updatePracticalItem(event){event.preventDefault();const f=event.currentTarget,id=f.dataset.trip,itemId=f.dataset.item,name=f.elements.namedItem('name').value.trim();if(!name)return;const type=f.elements.namedItem('type').value,details=f.elements.namedItem('details').value.trim(),address=f.elements.namedItem('address').value.trim();editingPracticalId=null;await savePracticalChange(id,items=>items.map(p=>p.id===itemId?{...p,name,type,details,address}:p))}
async function deletePracticalItem(id,itemId){await savePracticalChange(id,items=>items.filter(p=>p.id!==itemId))}
function renderTrip(id){
  ...
  const tabs=[['story',tr('tab.story')],['plans',tr('tab.plans')],['practical',tr('tab.practical')],['photos',tr('tab.photos')],['map',tr('tab.map')],['stamp',tr('tab.stamp')]];
...
<div class="tabBody" data-body="plans" hidden>${plansHtml(t)}</div>
<div class="tabBody" data-body="practical" hidden>${practicalHtml(t)}</div>
...
function openTrip(id){tripTabName='story';editingPlanId=null;editingPracticalId=null;navigate('#/trip/'+encodeURIComponent(id))}
...
async function saveEditor(){
  ...
  const trip=normalizeTrip({id:editingId||'trip_'+Date.now(),title,country:locations[0].country,cities:locations.map(l=>l.city),locations,start,end,story:$('fStory').value.trim(),tags:old?old.tags:['NEW'],plans:old&&Array.isArray(old.plans)?old.plans:[],practical:old&&Array.isArray(old.practical)?old.practical:[],photos:draft.map(({id,asset,src,thumb})=>({id,asset,src,thumb}))});
...
function stampsPanel(pe,n,next){
  const latestTrip=visited().filter(t=>t.start).sort((a,b)=>(b.end||b.start).localeCompare(a.end||a.start)||byStartDesc(a,b))[0]||null;
  const latestCountries=new Set((latestTrip?tripCountries(latestTrip):[]).map(countryKey));
  const stamp=(e,dashed)=>`<div class="stampWrap">${!dashed&&latestCountries.has(e.key)?'<span class="blotch"></span>':''}<button class="stampBtn" data-k="${esc(e.key)}" onclick="openCountry(this.dataset.k)">${stampSvg(e,{dashed})}</button>${!dashed&&e.visits>1?`<span class="visitBadge" aria-hidden="true">×${e.visits}</span>`:''}</div>`;
  ...
  return `${latestTrip?`<div class="stampLegend"><span class="stampLegendDot"></span>${esc(tr('stamp.latestTrip',{title:latestTrip.title}))}</div>`:''}<div class="stampCard">${pe.vis.map(e=>stamp(e,false)).join('')}${pe.up.map(e=>stamp(e,true)).join('')}${wish}${slot}</div><div class="blackCard passCard"><div><div class="capL">${esc(tr('pass.next',{n:next}))}</div><div class="nextTitle" style="font-size:18px">${esc(tr('pass.more',{n:next-n}))}</div></div><div class="pbar"><div class="track"><div class="fill" style="width:${Math.min(100,n/next*100).toFixed(0)}%"></div></div><div class="capL2 right">${n} / ${next}</div></div></div>`;
}
